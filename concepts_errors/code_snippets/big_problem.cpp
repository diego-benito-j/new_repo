...
void calcNucleotideFrequency(string sequence, map<char, int> &frequencies) {
    for (int i = 0; i <= sequence.length(); ++i) { // Off-By-One error, should be i < sequence.length() (THIS IS WHY WE GET ": 1" in the first line)
...
int main() {
    ...
    map<char, int> nucleotideFrequencies; // Should be deleted to avoid memory leak

    // Initialize frequencies for known nucleotides (A, C, G, T)
    nucleotideFrequencies["A"] = 0;
    nucleotideFrequencies["C"] = 0;
